...
namespace TempoIQ.Net.Connection
{
	...
	internal sealed class InternalHttpMessageHandler : DelegatingHandler
	{
		...
		public InternalHttpMessageHandler(HttpClientHandler innerHandler) : base(innerHandler ?? new WebRequestHandler())
		{
		...
		protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, System.Threading.CancellationToken cancellationToken)
		{
			if (request != null && request.RequestUri != null && !string.IsNullOrWhiteSpace(request.RequestUri.UserInfo))
			{
				request.Headers.Authorization = new AuthenticationHeaderValue(
                    "Basic", 
                    Convert.ToBase64String(Encoding.UTF8.GetBytes(request.RequestUri.UserInfo)));
			}
